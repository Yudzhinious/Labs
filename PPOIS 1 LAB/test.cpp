...
TEST(MultisetTest, RemoveElements) {
    ...
    EXPECT_EQ(multiset.getElements(), expected);
    ASSERT_EQ(output.str(), "Ýëåìåíò 'a' óìåíüøåí íà 1. Òåêóùåå êîëè÷åñòâî: 2\n");
    ...
    EXPECT_TRUE(multiset.getElements().empty());
    ASSERT_EQ(output.str(), "Ýëåìåíò 'a' óñïåøíî óäàë¸í.\n");
    ...
    ASSERT_EQ(output.str(), "Îøèáêà: ýëåìåíò 'a' íå íàéäåí â ìíîæåñòâå.\n");
}
...
TEST(MultisetTest, IsBracketInvalidCases) {
    ...
    ASSERT_STREQ(output.str().c_str(), "Îñòàëîñü 1 íåçàêðûòûõ '{'\n");
    output.str("");

    ASSERT_FALSE(isBracket(invalidSet2));
    ASSERT_STREQ(output.str().c_str(), "Îøèáêà\n");
    output.str("");

    ASSERT_FALSE(isBracket(invalidSet3));
    ASSERT_STREQ(output.str().c_str(), "Íå ïðàâèëüíûé ñèìâîë: [\n");
...
TEST(MultisetTest, ClearFunction) {
    ...
    ASSERT_EQ(output.str(), "Ìíîæåñòâî î÷èùåíî.\n");
}
...
TEST(MultisetTest, ShowBooleanFunction) {
    ...
    EXPECT_EQ(output.str(), "Íåò ïîäìíîæåñòâ äëÿ îòîáðàæåíèÿ.\n");
    ...
    EXPECT_FALSE(output.str().find("Ýëåìåíòû: d: 1") != std::string::npos);
}
